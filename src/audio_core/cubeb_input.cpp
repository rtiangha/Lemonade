...
void CubebInput::StartSampling(const InputParameters& params) {
    ...
    auto init_result = cubeb_init(&impl->ctx, "Lemonade Input", nullptr);
    ...
    auto stream_init_result = cubeb_stream_init(
        impl->ctx, &impl->stream, "Lemonade Microphone", input_device, &input_params, nullptr, nullptr,
...
std::vector<std::string> ListCubebInputDevices() {
    ...
    if (cubeb_init(&ctx, "Lemonade Input Device Enumerator", nullptr) != CUBEB_OK) {
