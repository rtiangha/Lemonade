...
void RO::LoadCRO(Kernel::HLERequestContext& ctx, bool link_on_load_bug_fix) {
    ...
    if (cro_buffer_ptr & Memory::LEMONADE_PAGE_MASK) {
    ...
    if (cro_address & Memory::LEMONADE_PAGE_MASK) {
    ...
    if (cro_size & Memory::LEMONADE_PAGE_MASK) {
