...
bool RasterFont::Initialize() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHARACTER_WIDTH * CHARACTER_COUNT, CHARACTER_HEIGHT,
                 0,
    ...
    glUniform2f(glGetUniformLocation(shader.handle, "charSize"),
                1.0F / GLfloat(CHARACTER_COUNT),
                1.0F);
...
void RasterFont::UpdateDebugInfo() {
    ...
    std::string text = fmt::format(
            "CITRA ENHANCED | FPS:{:>2} | SPD:{:>2}", static_cast<int>(stats.game_fps),
            static_cast<int>(stats.emulation_speed * 100.0));

    AddMessage(text, MessageType::FPS, Duration::FOREVER, Color::BLUE);

    /// This feature requires RasterFont re-boot, so don't available for now.
    /* if (stats.emulation_speed < 95) {
        AddMessage(text, MessageType::FPS, Duration::FOREVER, Color::RED);
    } else {
        AddMessage(text, MessageType::FPS, Duration::FOREVER, Color::BLUE);
    } */
}
...
void RasterFont::Draw(const Frontend::EmuWindow &window, const Layout::FramebufferLayout &layout) {
    ...
    draw_info.font_width = CHARACTER_WIDTH / draw_info.screen_width * 3.50f;
    draw_info.font_height = CHARACTER_HEIGHT / draw_info.screen_height * 3.50f;
