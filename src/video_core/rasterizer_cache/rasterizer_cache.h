...
void RasterizerCache<T>::ClearAll(bool flush) {
    ...
    for (auto& pair : RangeFromInterval(cached_pages, flush_interval)) {
        ...
        const PAddr interval_start_addr = boost::icl::first(interval) << Memory::LEMONADE_PAGE_BITS;
        const PAddr interval_end_addr = boost::icl::last_next(interval) << Memory::LEMONADE_PAGE_BITS;
...
void RasterizerCache<T>::UnregisterSurface(SurfaceId surface_id) {
    ...
    ForEachPage(surface.addr, surface.size, [this, surface_id](u64 page) {
        ...
        if (page_it == page_table.end()) {
            ASSERT_MSG(false, "Unregistering unregistered page=0x{:x}", page << LEMONADE_PAGEBITS);
            return;
        }
...
void RasterizerCache<T>::UpdatePagesCachedCount(PAddr addr, u32 size, int delta) {
    const u32 num_pages =
        ((addr + size - 1) >> Memory::LEMONADE_PAGE_BITS) - (addr >> Memory::LEMONADE_PAGE_BITS) + 1;
    const u32 page_start = addr >> Memory::LEMONADE_PAGE_BITS;
